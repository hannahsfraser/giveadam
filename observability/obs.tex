...
Questionable Research Practices (QRPs) are practices that make the final research outputs (papers, reports, presentations) look more certain or publishable than warranted by the methods used (Gould et al. in prep). This includes practices such as HARKing (hypothesising after results are known), p-hacking (this can include anything from deciding to exclude outliers after examing the results, to rounding p-values down to meet a threshold e.g. p<0.05), cherry-picking (conducting an undisclosed number of tests and only reporting the significant results), and not disclosing methodological problems (John et al ref), all of which are prevalent across fields of academic research (Fraser, Makel refs). These practices obstruct the ability of reviewers or end users to critically evaluate the merit and reliability of the described research but only cover one section of the research pipeline. The described QRPs are only relevant for the process of analysing results and describing the results. This excludes the swathe of important work that occurs prior to analysis. We argue that the discussion of influential and questionable practices should begin earlier, with consideration of practices regarding the documentation and performance of data pre-processing, transformation, and management. Poor implementation of these elements in the research pathway leads to misleading results (following the garbage in-garbage-out idiom). This is particularly problematic in multi-source data integration contexts, where harmonization decisions can significantly impact analytical outcomes. Further, documentation of these steps is inadequate across the suite of academic research (references to follow, Gould et al. in prep) meaning that no one reading the resulting research can detect whether these practices have occurred - just as in the case of the more commonly described Questionable Research Practices. We term problematic practices in the implementation and documentation of these early steps "Questionable Analytic Observations". Solutions to QRPs include increasing transparency with code, materials, and data sharing (ref) as well as preregistration to thwart HARKing, p-hacking, and cherry-picking (ref early article, Gould et al. https://ecoevorxiv.org/repository/view/9884). In theory, code, materials and data sharing are what is required to address Questionable Analytic Observations, but the current standard in documentation and sharing prior to the point of analysis is extremely limited (refs to find). A more comprehensive solution is required to address this problem.
